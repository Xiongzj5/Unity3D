...
public class Mouthposition
{
    ...
    public Mouthposition(float radius, float angle, float time) // 构造函数，构造的数据分别有半径、角度、时间
    {
        this.radius = radius;
        this.angle = angle;
        this.time = time;
...
public class ParticleMouth : MonoBehaviour {

    private ParticleSystem particleSys; // 粒子系统
    private ParticleSystem.Particle[] particleArr;  //粒子数组
    private Mouthposition[] circle; // 极坐标数组

    public int count = 10000; // 粒子数量
    public float size = 0.03f;// 粒子大小
    public float minRadius = 10.0f;// 最小半径
    ...
    void Start()
    {
        ...
        circle = new Mouthposition[count];
    ...
    void RandomFunc()
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            //随机每个粒子的角度
            float angle = UnityEngine.Random.Range(180.0f, 360.0f);
            float theta = angle / 180 * Mathf.PI;

            //随机每个粒子的游离起始时间
            float time = UnityEngine.Random.Range(180.0f, 360.0f);

            circle[i] = new Mouthposition(radius, angle, time);
    ...
    void Update()
    {
        for (int i = 0; i < count; i++)
        {
            ...
            // 保证angle在0~180度  
            circle[i].angle = (180.0f + circle[i].angle) % 180.0f + 180.0f;
